...
范围说明：
- SQLite 保存獨立瀏覽器會話；單進程本地 demo，尚未提供帳號登入或多進程並發控制，
...
def model_error(error):
    # Do not expose provider response bodies, keys or prompt contents.
    return jsonify({"error": "模型或工具暫時無法完成，本輪未保存，請重試。"}), 502
...
def chat():
    ...
    if not isinstance(payload, dict) or not isinstance(payload.get("message"), str):
        return jsonify({"error": "message 必須是文字"}), 400
