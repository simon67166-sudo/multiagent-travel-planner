...
15. ~~`web/nearby.js`/`web/index.html` 没跟着达人 Agent 重构改~~ **已解决（2026-09-15）**：走的是上面提到的第一个方向——`web/nearby.js` 整个删掉，`web/index.html` 里的独立附近游表单（城市/起点/日期/时长字段 + 提交按钮）、专属地图渲染代码（`drawNearbyMap`/`renderNearby`/`restoreNearby`）、相关 CSS 全部拿掉，左侧面板恢复成单纯的"实时日程"展示。`server.py` 的 `/nearby-plan`/`/nearby-weather`/`/nearby.js` 三个路由也从"留着但报错的 501 占位"改成彻底移除（路由表里已不存在，请求直接 404）。附近游现在完全走 `POST /chat` 自然语言（"从大三巴出发逛3小时"），排班结果写进主 `trip_plan`，`GET /trip` 自然带出来，前端不用维护第二套请求/渲染逻辑。真实浏览器验证过页面渲染干净，`tests/test_web_sessions.py` 同步更新（`test_nearby_endpoints_removed`，断言 404）
16. ~~`map_tool.route_between()` 对港澳场景两个挨得很近的真实地点偶尔查询失败~~ **已解决（2026-09-15），顺带修了地图坐标丢失的问题**：真实 demo 走查时发现两个相关问题——(a) 这一条记录的路线查询偶发失败；(b) `GET /trip` 的地图组件（`schedule_widgets.build_trip_map_widget()`）有时会把行程拆成好几张地图、点还落在离港澳很远的地方。根因是同一个：`trip_plan.add_stop()` 存节点时只存地点名字字符串，不存坐标——虽然 `content_agent.py` 早就用 `nearby_sources.find_origin()`/`nearby()` 查过真实、可靠的坐标了，但 `route_agent._place_day()` 写节点时把这些坐标扔了；后续不管是 `route_agent._real_leg()` 算交通时间，还是 `schedule_widgets.build_trip_map_widget()` 画地图，都得拿地点名字重新走一次 `map_tool.route_between()`/`map_tool.geocode()`——而这俩内部用的 `geocode/geo` 接口正是这个 session 已经证实对港澳不可靠的那个（同名地点解析到千里之外）。修法：①`route_agent._place_day()` 写节点时把 `lng`/`lat` 一起存进去（`trip_plan.add_stop()` 本来就支持 `**extra` 透传，不用改 `trip_plan.py`）；②`route_agent._real_leg()`/`schedule_widgets._route_leg_coords()` 坐标已知时改用 `nearby_sources.route(a, b, mode)`（早就存在但之前没人用的坐标直查接口，直接传经纬度算路线，不再地理编码）；③`build_trip_map_widget()` 拼地图标记时优先用节点上存的坐标，没有才退回按名字查。真实验证：`mode="nearby"` 排一次班，全程零次 `map_tool.geocode()` 调用、地图只生成 1 个框、`geocode_failures`/`route_failures` 都是空列表。老的 `route_agent.run()`（`server.py` 的 attraction_picker 确认流程还在用）当时保留了原来的按名字查询兜底行为——**但这条路径本身就是不可靠地理编码的活跃触发点，不是"没有回归"就完事了**：真实测试确认用户点"确认选择"之后，地图上出现过"上海""新疆"这种离谱定位。根因是 `server.py` 的 `apply_selection()` 把选中候选压缩成只剩 `place` 字符串（`item.get("place")`），content_agent 早就查好的 `lng`/`lat` 被丢弃，`route_agent.run()` 只能重新按名字地理编码。同一次修复（2026-09-15 第二轮）补上了这条路径：`apply_selection()` 改传完整候选字典，`route_agent.run()` 坐标已知时改用 `_real_leg()`（跟 `schedule()` 共用同一个坐标直查逻辑），没坐标才退回老的按名字查——`run()` 的 `__main__` 自测块等纯字符串调用方不受影响。真实验证：传真实候选字典，全程零次 `map_tool.geocode()` 调用
17. **`content_agent.run()`（mode="trip"）种子选择没有按地点去重**（2026-09-15 真实 demo 演示时发现）：`store.query_similar_posts()` 按人格相似度取 top_k*5 候选，只过滤掉 `category=="Tips"`，没有任何"同一地点只留一条"的约束——两条不同的社区帖子都在夸同一家店（真实测过"叠记咖喱美食"）完全可能同时挤进种子名额，变成同一个地方被推荐两次，LLM 组句时自己都发现了这个重复但没法从候选层面修掉。原有的 `seen_places` 去重（`content_agent.py` 里种子扩展周边那段）只防"种子 vs 周边扩展""周边扩展 vs 周边扩展"重复，没防"种子 vs 种子"重复。**已解决**：新增 `content_agent._dedupe_by_place()`，在 Tips 过滤之前先按 `place` 去重（`fetched` 已经按 `similarity_score` 降序排好，遇到的第一条就是相似度最高那条，后面同地点的直接跳过）。真实验证：同样的人格/城市查询，75 条推荐去重前后地点数一致（`len(places) == len(set(places))`），不再有重复。这一条跟第16条（坐标丢失）是同一轮真实 demo 演示发现的，但是两个独立根因，分开提交
18. **`route_agent.schedule()` mode="trip" 排班改成"时段骨架"，不再是纯最近邻贪心+一天11小时预算**（2026-09-15）：真实 demo 演示"帮我推荐一个澳门3日游"时用户发现两个结构性问题——(a) 每天推荐清一色是美食，没有景点；(b) 隔得很远的候选被排进同一天。根因：①`content_agent.run()` 的种子选择纯按人格相似度取 top_k，没有类别配额，人格标签偏"美食探店"时种子经常全是"饮食"类，每个美食种子扩展出的"周边真实 POI"大概率也还是餐厅，偏差被放大；②`route_agent._greedy_order()` 是纯最近邻贪心，没有单段距离/耗时上限，一天的时间预算是粗粒度的 11 小时，种子分散在城市不同区域时，某天贪心选完一个种子附近的候选后可能去抓另一个种子附近的候选，只要没超预算就会被排进同一天，跳跃很远。

**已解决**，用户提出的架构方向：编排 Agent（这里落在 `route_agent.py`，符合排时间接口归 route agent 的既定分工）先定好每天的"时段骨架"，路线 Agent 往里面填候选，槽位类型/时间窗口天然保证类别配比和距离约束：
- 新增 `_SLOT_TEMPLATE`：上午(09:00-12:00)/下午(14:00-17:30，可选)/晚上(19:30-21:30，可选)三个景点槽位 + 午餐(12:00-13:30)/晚餐(18:00-19:30)两个餐饮槽位，上午/午餐/晚餐是必选锚点。槽位之间是硬楼层（`cursor=max(上一段结束时间, 这个槽位start)`），不会因为上一段提前结束就提前开始下一段，刻意模拟真实旅游的节奏感。
- 候选按新增的 `_candidate_role()`（同时查"食"/"餐"两个字，覆盖社区帖子"饮食"分类和高德 POI"餐饮服务"分类两种来源——顺手修了 `_stay_minutes()` 原来只查"食"字、对高德餐饮 POI 大概率判不出来的隐藏 bug）分成景点/餐饮两个候选池，`_fill_day_skeleton()` 每个槽位从对应池子挑离当前位置最近、真实交通+停留时长能在槽位窗口内完成的候选（挑不到试下一个，最多试 3 个就放弃）；景点槽位排完主候选后，槽位还剩至少 45 分钟且候选池里有 800 米内的同角色候选，会再加塞一个（不算锚点）；全天节点数不超过 `_MAX_STOPS_PER_DAY=6`。
- 一天排完（还没真正写进 `trip_plan`）调一次 `_llm_review_day()`（`MODEL_LIGHT`，不是每加一站调一次——这个 demo 串行真实 API 调用已经要几分钟，逐候选调模型会让延迟更差）做"是否影响体验"的软性复核，只能拿掉加塞/可选槽位的候选，三个必选锚点不在可拿掉范围内（结构性保证的保底），LLM 调用失败/解析失败一律降级成"不拿掉"，不拖垮排班。
- `content_agent.run(mode="trip")` 种子挑选新增 `_pick_seeds_with_category_balance()`：优先保证至少 1 条"景点"类种子，不够就照常按相似度回退，不强求。

范围只改 `mode="trip"`；`mode="nearby"`（"从大三巴出发逛3小时"）现状良好，继续用原来的 `_greedy_order()`/`_place_day()`，没动。顺手修了一个不相关但相关代码路径里发现的显示 bug：`_place_day()`/新排班代码之前不管景点还是餐饮，写进 `trip_plan` 的 `type` 字段都硬编码成 `"attraction"`，前端（`web/index.html`）靠这个字段区分 🍜 餐饮图标和 📍 景点图标，餐饮类之前全部被错误显示成景点——现在按候选的真实角色写 `type`/`meal_type`。

真实验证：三天真实数据排班，有真实候选的那天正确混出"2个景点+2顿饭"结构（`type`/`meal_type` 字段正确），单段真实交通距离都在几百米以内，没有远距离跳跃；候选池数据本身覆盖不到的天（比如种子全是同一片区域的美食、附近查不到景点 POI）会诚实地只排出能排的（比如只有午餐晚餐），不会为了凑数硬塞不合理的候选——这是"229条社区帖子覆盖有限"这个已知数据局限的自然体现，不是算法 bug。地图 widget 复测仍然是 0 次 `map_tool.geocode()` 调用。**已知局限**：槽位时间是写死的模板，没有根据用户说的"轻松点"/"紧凑点"调节奏；候选池天生只有美食（人格向量/该城市数据分布导致一条"景点"都检索不到）时，景点槽位会一直空着，`_pick_seeds_with_category_balance()` 只能在候选池里"确实有景点"的前提下起作用，不能凭空生成候选

19. **`ota_hotel_agent` 酒店搜索不管已排行程的位置，无差别搜整个城市**（2026-09-15 真实 demo 演示时发现）：用户问完 3 日游行程后接着问"帮我订酒店"，编排 Agent 兜底的自由文本组句"聪明地"看着 `trip_plan` 里的行程位置建议住哪个区域（比如"你的行程都在新马路一带，别住氹仔"）——但那只是模型看 JSON context 自由发挥，`ota_hotel_agent._search_real_hotels()` 真正调 `hotel_tool.search_hotels()` 时一直传 `place=city, place_type="城市"`（`orchestrator_agent.py` 调用时也是 `location=None`，完全没把已排的行程位置传进去），从来没按行程收窄过——嘴上说的和真正会出现在 `hotel_picker` 卡片里让用户选的候选不是一回事，用户点卡片照样能选中文字里劝退的氹仔酒店。真实验证过：同样查询条件下，`place="澳门", place_type="城市"` 的结果里有氹仔的"英伦游艇会酒店"，`place="大三巴", place_type="景点"` 的结果里没有。

**已解决**：新增 `ota_hotel_agent._anchor_place_from_trip()`，从 `shared_state["trip_plan"]` 里取最早一天第一个带坐标的站当"锚点"（`{"place":,"lng":,"lat":}`，坐标是排班时就存好的真实坐标，见第18条），`_search_real_hotels()` 有锚点就传 `place=锚点地点, place_type="景点"`（没有就退回原来的 `place=city, place_type="城市"`）。**踩了一个新坑**：锚点不够出名时 RollingGo 会整个匹配错地方——真实测过"中西药局旧址"（澳门一处不太热门的历史建筑）被匹配到美国圣路易斯，坐标直接跑去密苏里州。加了一道合理性校验：用锚点已知的真实坐标跟返回的酒店坐标算距离，超过 `_ANCHOR_SANITY_KM=50` 公里就当整批匹配失败，自动退回城市级搜索重查一次，不把跑偏的结果直接返回给用户。**已知局限**：只有锚点本身是知名地标（比如"大三巴"）时才能可靠收窄，社区帖子/高德 POI 里大量不那么出名的地点（历史建筑、小吃店）大概率触发合理性校验退回城市级搜索——退回去之后行为等同于改动前，不会更差，只是没能享受到收窄的好处；不强求每次都收窄成功，符合项目一贯"能收窄就收窄，收窄失败就老实退回"的降级哲学。

20. **聊天框把编排 Agent 的 Markdown 回复当纯文本显示，`##`/`**`/`|表格|` 这些符号原样露在外面**（2026-09-15 用户反馈）：`web/index.html` 的 `addMessage()` 一直是 `div.textContent = content`，编排 Agent 通用组句（`orchestrator_agent.py` 里没命中 `restaurant`/`cancel` 时走的 LLM 自由组句分支）经常输出结构化的 Markdown（标题/加粗/表格/列表），用户看到的是一整段带着原始符号、没有任何格式的文字。**已解决**：引入 `marked.js`（`cdnjs` CDN，`v9.1.6`，纯前端库，不需要改后端），`addMessage()` 对 `role==="assistant"` 的字符串内容改成 `marked.parse()` 转出 HTML 再塞 `innerHTML`（`user`/`system` 角色继续用 `textContent`，用户自己打的字没必要也不该被当 HTML 渲染）；`sendMessage()` 里单独处理 `pendingEl` 的那处直接赋值也一并改了。`marked` 没加载成功（比如断网、CDN 被拦）会优雅退化回纯文本，不让聊天框崩掉。配了一套 `.msg.assistant` 下的标题/表格/列表/引用样式，不是用浏览器默认样式硬堆在聊天气泡里。真实验证：用户之前反馈的那段酒店推荐 Markdown 原文重新走一遍渲染，表格/加粗/分级标题/列表全部正常显示。

21. **编排 Agent 组句回复的行程跟 `trip_plan` 实际排定的不一样**（2026-09-15 用户反馈：文字里 Day2 的行程跟左侧面板实际排的 Day1 对不上）：`orchestrate()` 通用组句分支（`else` 兜底那段）把 `shared_state["trip_plan"]` 原始链表结构（`{"head_id":,"nodes":{id:{...,"next_id":}}}`）连同 `results.content.recommendations` 全量候选池（真实测过 75 条，带 `community_reviews`/`images`/`tags` 等重字段）一起塞进 context，模型很难正确按链表顺序转述每天的行程，加上候选池信息量太大，容易把"候选"和"已经排定的行程"搞混，自己把候选重新编排一遍，跟 `route_agent.schedule()` 真实排的顺序/天数对不上。**已解决**：`trip_plan` 改传 `trip_plan.render()` 之后的整理版（`{"days":[{"date":,"stops":[有序数组]}],...}`），`recommendations` 新增 `_trim_recommendations_for_reply()` 精简成只剩 `place`/`category`/`source`；system prompt 里明确加了一句"`trip_plan.days` 是已经真实排定的行程，必须按它如实转述，不能自己重新编排；`recommendations` 里没排进 `trip_plan.days` 的地点只能提成备选"。真实验证：重新跑一次"9.15-9.18 澳门3天"的真实请求，回复文本的三天地点/顺序/时间跟 `trip_plan.render()` 逐字对上，未排入的候选老实标成"备选地点（还没排进行程）"。

22. **排时间的触发点从"意图分类猜中了 route"改成"用户从 `attraction_picker` 选完确认"**（2026-09-15）：真实测试发现同一句"帮我推荐一下怎么玩呗"排不排班完全取决于 `classify_intent()`（一次真实 LLM 调用）有没有把这句话分进 `"route"` 意图，同一句话不同轮次分类结果可能不一样，导致"这次有行程表、下次没有"；而且原来自动排班用的是 `content_agent` 算出来的全量候选池（真实测过 75 条），用户还没表达任何偏好，算法就自己把整个行程定了。**已解决**：`orchestrate()` 不再有"`content` + `route` 意图 → 自动排 `mode="trip"`"这条分支（`mode="nearby"` 不受影响，"从大三巴出发逛3小时"这种直接行动请求命中就照常立刻排，它没有"先给候选、等用户挑"这一步）；`classify_intent()`/`_INTENT_SYSTEM_PROMPT` 里 `route` 意图整个删除（改动后没人用了）；`server.py` 的 `apply_selection()` 的 `attraction_picker` 分支从老接口 `route_agent.run()` 改调真正的骨架排班 `route_agent.schedule()`（`mode="trip", time_budget_days=1`）——用户选完确认才是第一次真正排班的时刻，且用的是用户自己选的候选，不是算法全池子自动决定的。`attraction_picker` 选择上限从 3 放宽到 6（`widgets.py`，对齐 `_SLOT_TEMPLATE` 一天最多 6 个节点的上限），候选池 `pool_size` 从 6 调到 10（给用户留出真正能挑的空间）。多天行程靠多轮"推荐→选→确认"自然累积（`schedule()` 本来就会找第一个没用过的 `day-N` 续排，不需要新状态）。老接口 `route_agent.run()`/`_estimate_transport()` 改完之后彻底没有调用方了，直接删除（连同 `orchestrator_agent._extract_day_count()`，唯一调用点被删掉后也成了死代码）。真实验证：`POST /widget-response` 走 `attraction_picker` 确认真实候选，确认 `route_agent.schedule()` 被调用、`trip_plan` 首次被填入、零次 `map_tool.geocode()` 调用。

23. **达人 Agent 新增候选分类质检，标错了顺手改数据库**（2026-09-15，用户提出）：`route_agent._candidate_role()` 靠"category 字段里有没有'食'/'餐'字"分餐饮/景点，本质是信任上游数据——社区帖子的 `category` 是人工录入的，可能真的标错（比如一家餐厅被手误标成"景点"），标错一条就可能把餐厅排进景点槽位，绕开骨架排班"类别配比"这个结构性保证。**已解决**：`content_agent.py` 新增 `_verify_and_fix_categories()`，候选提交给编排 Agent 之前，一批候选**一次性**打包成一次 `MODEL_LIGHT` 调用做质检（不是逐条调用，控制延迟），发现标错的就地修正、带 `post_id` 的（社区帖子来源）顺手调新增的 `store.update_post_category()` 写回数据库——**验证过 Chroma 的 `update()` 是合并式更新**，只改 `category` 这一个字段，`place`/`caption`/`tags` 等其他字段原样保留，不会因为一次分类修正把整条帖子的其他数据冲掉。高德 POI 来源没有持久化数据可写，只改这次返回结果的内存字段。LLM 调用/解析失败就什么都不改，不影响 `content_agent.run()` 主流程。真实验证：229 条真实数据跑一次没有触发误报/崩溃（这批数据本身没查出真实标错的，属于正常情况，质检逻辑本身用真实 Chroma 读写单独验证过）。

24. **酒店锚点改成"多天行程末站重心" + 已订酒店的距离复核**（2026-09-15，用户提出）：第16条加的 `ota_hotel_agent._anchor_place_from_trip()` 原来只取"最早一天第一个带坐标的站"当酒店搜索锚点——多天行程时只照顾了第一天，比如 Day3 行程跑去氹仔，酒店搜索完全没考虑到。用户指出订酒店应该看**每天最后一站**（晚上回酒店睡觉，最后一站离酒店远不远才是真正影响住宿体验的），且已经订好的酒店只要不是特别远就不用换。**已解决**：
    - 新增 `_days_last_stops()`：取每天最后一个带坐标的节点（不是第一个）。
    - `_anchor_place_from_trip()` 改成取**所有天数最后一站坐标的重心（平均值）**，不再只看某一天；RollingGo 的 `place` 参数要真实地名，用离重心最近的那一站地点名字做文字检索，但坐标本身用重心点（后续合理性校验更准确地反映"整趟行程"）。
    - 新增 `_existing_hotel_distance_check()`：已经订好酒店（`trip_plan.hotels` 非空）时，检查现有酒店离每天最后一站够不够近（`_HOTEL_CONVENIENCE_KM=5` 公里内不用管），超出的天数只在回复文字里提醒（`result["hotel_distance_reminder"]`），**不自动重新搜索/推荐换酒店**——酒店是用户已经确认的真实决定，系统不该擅自建议换。
    - `run()` 里：已经订了酒店就不再发起新的酒店搜索（`booking` 意图每次命中都重新搜一遍候选会显得像是在暗示用户换酒店，不合理），改成只做距离复核。
...
26. **新增行程偏好确认：搜索前先问酒店/机票偏好，跟 persona 已有维度交叉引用**（2026-09-16，用户提出）：用户希望编排 Agent 在真正调用达人 Agent 搜索之前，先确认旅行模式、酒店需求、机票倾向、旅行节奏，做到更精准推荐；并问能不能跟 persona 系统交叉引用。**查过 `persona.py` 确认**：四个维度里三个已经存在——"旅行节奏"=`pace_score`、"度假 vs 出差"=`scenario`、"逛吃美食 vs 景点打卡"=`interest_theme` 标签，都不需要重新问/重新存。真正没被覆盖的只有**酒店偏好**（周边图方便 vs 单独选一家好酒店当度假体验，不等于 `budget_score`）和**机票优先级**（省钱/折衷/极致体验）。**已解决**：
    - 新增 `orchestrator/trip_preferences.py`：`TripPreferences` 数据类只存这两个新字段，`describe_for_persona()` 从 persona 读"节奏"/"模式"给调用方用，不重复存一份（避免同一个概念存两处、改一个忘了改另一个）。
    - `orchestrator_agent.py`：`new_shared_state()` 加 `trip_preferences`/`pending_trip_request`（暂存"问偏好之前用户真正想说的那句话"）两个字段；`orchestrate()` 开头新增确认流程——`content` 意图命中且 `nearby` 未命中（即 mode="trip" 常规推荐场景）时，若偏好没问完，**一次性**列出所有缺失维度的追问（用户已用 AskUserQuestion 确认过是一次问完不是分好几轮），把原始请求存进 `pending_trip_request`，短路返回；下一轮如果 `pending_trip_request` 非空，就把这轮消息当回答解析（`_parse_preference_answer()`，一次 `MODEL_LIGHT` 调用，用户说"继续"/解析不出来的字段用默认值兜底，不会卡住对话），然后取出暂存的原始请求继续走正常分发。`mode="nearby"`（"从大三巴出发逛3小时"）不需要先确认，跟它不走 `attraction_picker` 确认流程是同一个道理。
    - `ota_hotel_agent.py`：`hotel_preference="度假酒店"` 时不走 `_anchor_place_from_trip()` 的锚点收窄（用户不介意离行程远），改成城市级搜索 + 提高星级门槛（`_search_real_hotels()` 新增 `star_min` 参数透传给 `hotel_tool.search_hotels()`，那边接口本来就支持，之前没接上）；`flight_priority="省钱"` 时把假机票候选按价格升序排——**如实说明局限**：机票是假数据（`_generate_beijing_flights()`），`flight_priority` 目前只能影响候选排序，不能真正筛选时刻/舱位。

真实验证：真实两轮对话，第一轮"推荐一个3天的澳门行程"触发确认（`community_panel` 为空，`pending_trip_request` 正确存住原始请求）；第二轮"周边方便就行，机票随便"正确解析出 `{"hotel_preference": "周边", "flight_priority": "折衷"}`，用暂存的原始请求真正触发了搜索（290 条候选），回复文本里正确引用了 persona 的 `interest_theme`/`pace_score`（"自然风光、美食探店、家庭出游、节奏偏慢"）。

27. **周边 POI 扩展没有筛选/截断，候选量爆炸**（2026-09-17，用户反馈"周边POI也太多了"）：上一条验证时"3天澳门行程"实测出了 290 条候选，根因是 `content_agent._nearby_plan()` 对 `nearby_sources.nearby()`（AMap 同城 POI 检索，一次能回几十条）的结果完全不截断——`run()` 里 mode="trip" 场景对每个种子（第25条硬配额下，3 天行程最多能有 2×3+3+2×3=15 个种子）都各自调一次 `_nearby_plan()`，汇总去重后自然是几百条量级，两轮 `_classify_categories()` 的 LLM payload 也跟着臃肿。用户建议参考已删除的 fellow 分支 `nearby_planner.py`（`git show <deletion-commit>~1:orchestrator/nearby_planner.py` 找回来看的）——那边的做法是先用一次 LLM 调用把候选"从资料中选出最多6个地点"（`rank_candidates()`），再 `pool = ranked[:8]` 硬截断到 8 个，才进贪心排线路。**已解决**：`_nearby_plan()` 新增 `limit` 参数（默认 `_NEARBY_PER_CALL_CAP=8`，直接对齐 fellow 分支当年截断到 8 的量），按已有的 `persona_match_score`/`recommendation_score`（口碑复核算出来的排序分）排序后截断——跟 fellow 分支的目的一样（"别把一次周边查询的全部结果都塞给下游"），但不需要 fellow 那一步额外的 LLM 排序调用：那边当年没有现成的排序依据，只能靠 LLM 挑；现在口碑复核已经算出了结构化分数，直接按分数截断效果等价，还省一次 LLM 调用和一次可能的解析失败点。真实验证：同样"我想在9.15从北京出发去澳门玩3天"的真实请求，候选总数从 290 降到 58~66（景点/午晚餐类别配额依然满足，早餐类目前为 0 条——查证是社区帖子库里澳门本来就没有早餐类型的帖子，第25条配额"尽力而为不强求"的既有降级行为，跟这次截断改动无关，不是新引入的回归）。

28. **`attraction_picker` 确认排班只喂用户勾选的那几个候选，达人 Agent 找到的其它候选被晾在一边**（2026-09-17，用户提出"难道编排 Agent 不应该把所有达人选出来的给路线 Agent 吗，只是用户选了的优先级更高"）：第22条把排时间的触发点从"意图分类"改成"用户在 `attraction_picker` 选完确认"之后，`server.py` 的 `apply_selection()` 一直是 `route_agent.schedule(state, picks, ...)`——`picks` 只是用户在候选卡片里勾的那几个（最多 6 个，而且候选卡片本身就只展示 `widgets.build_attraction_picker_widget()` 精选出的 `pool_size=10` 个，不是达人 Agent 的全量候选池）。`route_agent.schedule()` 排班时只看得到这几个候选，早餐/午晚餐/用户没勾的其它景点槽位完全没有候选可用，只能空着——不是"用户选的优先"，是"用户没选的直接不存在"。**已解决**：
    - `orchestrator_agent.py`：`orchestrate()` 生成 `attraction_picker` widget 的同时，把达人 Agent 给的全量 `candidates` 存进 `shared_state["last_content_candidates"]`（`new_shared_state()` 也加上这个默认字段）——`attraction_picker` 的确认是另一次独立的 HTTP 请求（`POST /widget-response`），到那时候本轮 `candidates` 早就不在调用栈里了，必须存进 session 状态才能跨请求拿到。
    - `route_agent.schedule()` 新增 `priority_places` 参数（候选 `"place"` 字段的集合），透传给 `_fill_day_skeleton()`：槽位内候选试探顺序从纯按距离排序，改成 `(是否命中 priority_places, 距离)` 二级排序——命中的候选在同池子里永远排最前面优先尝试（排不进当前槽位再轮到后面按距离排的其它候选），不是"只排这些"。
    - `server.py` 的 `apply_selection()`：`attraction_picker` 分支改成把 `state["last_content_candidates"]`（全量候选池，取不到就退回只用 `picks`，不炸）交给 `schedule()`，用户勾选的 `picks` 转成 `priority_places` 传进去；衔接回复（见下一条）改成检查用户勾选的地点有没有真的排进去，不是检查全量池子的剩余数量（全量池子本来就有几十条，绝大多数因为一天排班槽位有限进 `unscheduled` 是正常现象，不是"没排上"）。

29. **`attraction_picker`/`flight_picker`/`hotel_picker` 确认之后聊天框没有任何反应**（2026-09-17，用户反馈"能不能做完交互后加点衔接提示"）：`POST /widget-response` 一直只返回 `{"status":"ok"}`，`trip_plan`/地图面板悄悄更新了，但聊天框只有前端自己拼的一句"已确认：地点1、地点2"，用户看不出排班/订票订房这些后台动作到底成没成功。**已解决**：`apply_selection()` 现在返回一句话总结实际发生的事——景点排完之后说排进了哪天，用户勾的地点有没有全部排上、没排上的具体是哪几个；机票/酒店订完之后说订好了什么，酒店额外带上 `_existing_hotel_distance_check()` 的距离提醒（原来只有聊天流程接了这个提醒，widget 流程没接）。这句话存进 `state["messages"]`（下次 `/session` 刷新还在）也直接回传给前端这次响应用，前端 `renderPicker()` 确认成功后除了原来的"已确认：..."系统提示，多显示一条助手消息气泡。

30. **`_extract_day_count()` 日期区间算成了"晚数"，少算一天**（2026-09-17，用户指出"9.15到9.18其实是4天"）：第27条刚加的日期区间兜底用 `(结束日期-开始日期).days` 算天数，9.15到9.18算出来是3——这是两个日期之间的"晚数"，中文口语"4天3晚"里的"天数"要把首尾两天都算进去，得在晚数基础上 +1。**已解决**：日期差 +1 再封顶 14 天；同时把这次改动跟第26条"显式`N天`优先于日期区间兜底"的顺序验证了一遍，没有互相影响。

31. **`route_agent.schedule()` 排班没有地理聚类，也不知道机票时间**（2026-09-17，用户提出两点：① "排路线也该把离得比较近或者顺路的排一天"，建议三步法——先按 `recommendation_score` 选排名靠前的候选，再按距离/通勤时间聚类分天，最后照旧塞加塞候选；② 机票的到达/离开时间要嵌入排班，落地当天/离开当天不能套用完整一天的时段骨架，且"用户选中的景点如果被冲掉了要挪到别的天，按距离看哪天最合适"）。原来 `_fill_day_skeleton()` 是全行程共享一个大候选池，按天贪心从槽位第一个开始排，同一天的候选之间没有"地理上该扎堆"的约束，也完全不知道有没有订机票、机票几点起降。**已解决**，`route_agent.py` 新增/改动一批函数：
    - `_centroid()`/`_greedy_geo_cluster()`/`_assign_by_proximity()`：三步法的第①②步。景点候选先按 `recommendation_score` 排名当"种子"，贪心聚成 `time_budget_days` 组（同一天的候选地理上尽量扎堆，不是天南地北乱跳）；早餐/午晚餐按"离哪天的景点聚类重心最近"分天，保证顺路，不会"早餐城北、景点城南"。`priority_places`（用户在 `attraction_picker` 里勾选的地点）会优先当聚类种子，正常排队还是没能分到任何一天（比如所有天容量加起来都不够，或者被机票时间挤没了槽位）会有兜底：硬塞进离它最近、已经有候选的那一天，哪怕因此超出这天原定容量——用户明确选的地点不能因为排班算法的配额算不过来就消失在 `unscheduled` 里。
    - `_flight_arrival_and_departure()`：从 `trip_plan.flights` 里找"飞到本次城市"的到达时间、"从本次城市飞走"的起飞时间（demo 机票数据没有专门的 direction 字段，用 `to_`/`from_` 跟城市名比对判断方向，够用不追求精确）。
    - `_clip_slot_template()`/`_day_slot_templates()`：把抵达当天的槽位窗口裁到不早于"到达时间+2小时缓冲"（过关取行李通勤），离开当天裁到不晚于"起飞时间-3小时缓冲"（去机场值机安检）——是**裁窗口**不是整槽丢弃：落地10:10、缓冲到12:10，"午餐"槽位名义上12:00开始、窗口到13:30，裁完变成12:10-13:30还留得住，不会因为名义开始时间比这天能动的时间早10分钟就整个作废。
    - `_fill_day_skeleton()` 新增 `slot_template` 参数（默认还是完整的 `_SLOT_TEMPLATE`），`schedule()` 给抵达/离开当天传裁剪过的版本，中间的天不受影响；这天砍到一个槽位都不剩（比如当天来回）就原样不排，不报错。
    - `schedule()` 把景点/早餐/午晚餐三个池子的"每天容量"从写死的常数改成按 `_day_slot_templates()` 算出来的每天实际槽位数——抵达/离开当天容量自然变小，聚类那一步提前知道这天装不下多少，不会先分多了再被槽位挡回去。

    真实验证：已订机票 20:00 落地（缓冲后当天景点容量变 0），用户在 `attraction_picker` 里勾的一个景点排 2 天时被正确挪到了 day-2（没有消失在 `unscheduled` 里）；两组地理上隔得很开的景点候选（模拟"城中"/"路环"）排 2 天行程时确实各自成组，没有拆开混进同一天。

32. **机票候选带 `date` 字段，存进 `trip_plan.flights` 时手动拼字段漏掉了，前端也没渲染**（2026-09-17，用户反馈"机票展示这一栏没有日期这不合理"）：`ota_hotel_agent.run()` 生成的机票候选其实一直带着 `"date"`（去程用入住日期、回程用离店日期），但 `server.py` 的 `apply_selection()` 的 `flight_picker` 分支手动拼字典存进 `trip_plan.add_flight()` 时漏了这一项，候选有、存进去的没了；就算存了，前端 `web/index.html` 的机票卡片渲染也压根没有引用 `f.date`。**已解决**：两处都补上——`server.py` 存的时候带上 `"date": item.get("date")`，前端卡片的 `.sub` 那行加上日期显示。`trip_plan.add_flight()` 的字段说明文档字符串也顺手更新（之前没提这个字段）。

33. **订酒店地图上出现了异地酒店**（2026-09-17，用户真实预订时反馈）：`ota_hotel_agent._search_real_hotels()` 的锚点合理性校验（第19条加的，挡"中西药局旧址被匹配到美国圣路易斯"这类跑偏）有个真实 bug——`on_target`（离锚点 `_ANCHOR_SANITY_KM` 内的候选）算出来只用来判断"要不要整批放弃重查"，判断完之后却没拿这个过滤结果替换 `hotels`：5 个候选里只要有 1 个在范围内就不会触发重查，但混进最终候选的还是**没过滤的原始 5 个**，跑偏的酒店照样会跟着正常的一起出现在地图上。另外，没有 `anchor` 的城市级搜索（比如 `hotel_preference="度假酒店"` 场景）之前压根没有任何坐标合理性校验，RollingGo 城市级匹配理论上也可能出问题，只是没人踩到过。**已解决**：
    - `on_target` 非空时真的用它替换 `hotels`（`hotels = on_target`），不是算完就扔，跑偏的候选不会再混进最终结果。
    - 新增 `_city_center(city)`：用 `map_tool.geocode(city)`（真实地理编码，城市名是标准行政区划名字，不是生僻地标，可靠）查城市中心点，查不到就返回 `None`，调用方跳过校验不强求。
    - 新增 `_CITY_SANITY_KM=80` 兜底校验：不管走没走锚点，最后都会拿结果跟城市中心比一次距离——这层校验跟锚点校验管的不是同一件事（锚点校验管"离行程地点近不近"，这个管"压根有没有查到别的城市/国家去"），哪怕锚点校验已经通过，也不代表整个查询没有串到别的城市。

    真实验证：模拟"5个候选混着1个异地酒店"的场景，锚点校验/城市级校验都能正确把异地酒店过滤掉，只剩本地候选；`_city_center()` 查不到时优雅跳过校验，不报错、不误杀。

34. **酒店/机票偏好问过一次就全程不再问，但用户指出这是"这一趟去哪儿"的定制，不是绑在人身上的人格变量**（2026-09-17，用户提出"去福州可能更想吃，去香港可能更想打卡，去岘港可能想度假，每次都不一样"）：第26条加的 `trip_preferences` 门槛只看 `prefs.is_complete()`，问完（或者用户"继续"用默认值兜底）之后这个 session 里不管后面请求哪个城市都不会再问——隐含假设是"这两个偏好是跟人绑定的"，但用户指出这两个字段其实是跟"这一趟行程"绑定的，换目的地就该重新问。**已解决**：`new_shared_state()` 新增 `trip_preferences_city` 字段，记录当前 `trip_preferences` 是为哪个城市答的；`orchestrate()` 里这次请求提取到的城市（`_extract_city()`，跟 `content_agent.py` 独立一份同样的子串匹配思路）如果跟这个字段不一样，就把 `prefs` 整个重置成空白（两个字段都重新问，不是只问少的那部分——上次答案是为另一个城市答的，沿用没有意义），重新触发确认门槛。提取不到城市（这句话没提、`shared_state["city"]` 也没有）就不重置，避免无谓打断。追问期间暂存的 `pending_trip_request` 旁边新增 `pending_trip_city`，等用户答完才真正写进 `trip_preferences_city`，没答完之前不会污染这个记录。

    真实验证：真实两轮对话，先在"澳门"问完确认答成"度假酒店/省钱"；第二轮请求"香港"的行程，确认门槛正确重新触发（不是直接沿用"度假酒店/省钱"），`trip_preferences` 被重置成空白。

35. **新增：机票酒店行程都定完之后调 `exception_agent` 查真实天气，标出行程里可能下雨/下雪的天，雨大就问要不要调**（2026-09-17，用户提出）：`exception_agent.py` 之前只有 `run()`（子串匹配"这句话有没有提到行程里的地点"）和 `check_weather()`（查某个城市当前有没有真实生效的灾害预警），都不是"给已经排定的每一天分别查天气、标进行程"这种颗粒度。新增：
    - `_day_dates_from_flight(shared_state)`：`trip_plan.days` 的 key（`"day-1"`/`"day-2"`...）是 `route_agent.schedule()` 排班时按顺序分配的占位符，不是真实日历日期（已知的设计取舍）——借去程航班的真实 `date` 字段（第32条刚补上的）倒推，`day-1` 是航班抵达那天，`day-N` 顺推 `N-1` 天。这也是为什么这个功能必须等机票订完才能查：没有真实日期，没法把 `weather_tool.get_hourly_forecast()` 查回来的真实日历天气跟"day-N"这种占位符对上。
    - `check_itinerary_weather(shared_state)`：对 `trip_plan.days` 里每一天（按 `_day_dates_from_flight()` 换算出真实日期后）查一次天气预报，按这天最后一个有坐标的节点查（跟 `route_agent.schedule()` 排完一天查天气用的是同一个"查最后一个点"思路，不用每站都查）。降雨概率或者有降雪的天，写一条进 `trip_plan.weather_alerts`（前端本来就在渲染这个字段，不用改前端）；降雨概率达到阈值（跟 `route_agent.schedule()` 的降雨提醒用的是同一个 0.6）就整体标 `needs_replan=True`，返回一句"要不要调整"的问句——这是**纯信息性标记**（不删除/修改任何行程节点），不受这个模块"提案制、不自动执行"限制里"会真的改变行程结构的操作"那条规则约束；真要怎么调整还是得用户自己在对话里说清楚，不猜、不自动执行。
...
37. **删除 `restaurant_agent`（餐厅演示技能）——不接真实数据，还会把整段对话卡死在死胡同里**（2026-09-17，用户拿到团队成员真实测试的聊天截图，发现对方问"那按9/15-9/17的3天2晚来规划"这种纯粹推进日期确认的话，被分类成了 `restaurant` 意图，之后整段对话再也没跳出来——一路生成越来越像真的"3天2晚参考草案"（带具体航班号/酒店名/演示餐厅），但 `trip_plan.days` 全程是空的，左侧"还没有行程安排"贯穿了 4 张截图）：
    - **查实根因有两层**：① `restaurant_agent.py`（`fellow` 分支早期写的）查的是 `restaurant_tools.py` 里硬编码的 3 条"演示餐厅 A/B/C"，从来没用过 `store.py` 的真实社区帖子库——跟这个项目现在"真实数据不瞎编"的其它部分（真实天气/真实酒店/真实路线/真实候选）完全不是一回事，纯属早期占位从没升级；② `_INTENT_SYSTEM_PROMPT` 里 `restaurant` 那条有一句"包括前文餐饮需求的修改、追问与回忆"，本意是让用户能连续调整餐厅筛选条件不用每次重新说"推荐餐厅"，但太宽泛——只要历史里出现过餐饮相关内容，LLM 就容易把后续任何模糊的推进类消息也当成"餐饮需求的追问"粘滞进去；`orchestrate()` 里 `elif "content" in intents and "restaurant" not in intents:` 这行还会让 `restaurant` 一旦命中就直接跳过 `content_agent`，哪怕两个意图同时命中也不例外。
    - **用户确认**：既然这个技能根本不用真实数据库，直接删掉，不用修。**已解决**：整个删除 `orchestrator/agents/restaurant_agent.py`、`orchestrator/restaurant_tools.py`；`orchestrator_agent.py` 不再 import/调度它，`_INTENT_SYSTEM_PROMPT` 去掉 `restaurant` 这一条（`content` 的说明顺带改成"景点或美食推荐"，明确美食推荐现在完全由 `content_agent` 的真实数据覆盖）；`classify_intent()` 的白名单也去掉 `restaurant` 这个 key——就算 LLM 因为训练偏置还是吐出这个词，也会被过滤掉，不会走到任何特殊分支，从根上让这类"卡在死胡同"的情况不可能再发生。回复组句、`shared_state["messages"]` 写入逻辑也一并简化（去掉之前专门为 `restaurant_agent` 自己管理对话历史留的特殊分支）。
...
39. **候选池里真的出现了"公厕"当景点——查到根因是高德自己的数据标错了类别**（2026-09-17，用户真实反馈候选里有个叫"公厕"的"景点"，"这个明显不适合作为景点"）：之前几次查这个问题都没查到实锤（社区帖子库里唯一相关的帖子正确标着 `Tips`），这次用户给出的是当前真实 session 里的候选，直接从 `last_content_candidates` 里翻出了真凶——一条 `source: "高德POI"` 的候选，`place: "公厕"`，`category: "景点"`。直接拿这个坐标真实查了一次高德周边接口，坐实了根因：高德自己给这个"公厕" POI 标的 `type` 是 `"风景名胜;风景名胜;风景名胜"`（`typecode: 110200`）——**高德自己的数据就把这个公共厕所归类成了"风景名胜"**，`_classify_categories()`（第25条加的分类质检）看到这个"风景名胜"字样，顺理成章地把它归成了"景点"。这不是分类提示词的锅——分类环节能判断的是"这条候选属于哪个类别"，没法凭空判断"这条候选压根就不该被推荐"，上游数据源自己标错类别这种情况，指望分类提示词纠正不现实。**已解决**：真正可靠的信号是地名本身，不是类别字段——`content_agent.py` 新增 `_NON_RECOMMENDABLE_PLACE_KEYWORDS`（公厕/洗手间/卫生间/停车场/收费站/公共电话亭），`_nearby_plan()` 查回真实高德 POI 之后，地名命中这些关键词的直接排除，不进候选池，不用等分类环节再去猜——从"查回来"这一步就拦住，比"归类完再判断要不要剔除"更早、更省事。

    真实验证：拿真实坐标（庇山耶街26号附近）重新跑一次 `_nearby_plan()`，"公厕"已经不在候选结果里了。89 个测试全过。
